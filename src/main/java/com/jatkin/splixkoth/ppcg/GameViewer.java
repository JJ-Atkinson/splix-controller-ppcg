...
public class GameViewer implements GameNode {
    ...
    MutableMap<SplixPlayer, Color> playerColors;
    GraphicsContext g;
    ...
    public GameViewer(Canvas canvas, Supplier<SplixBoard> board, Function<SplixBoard, SquareRegion> getView, MutableMap<SplixPlayer, Color> playerColors) {
        this.canvas = canvas;
        this.board = board;
        ...
        for (int y = offsetY; y <= region.getTop(); y++) {
            for (int x = offsetX; x <= region.getRight(); x++) {
                ...
                if (board.getBounds().inBounds(loc)) {
                    ...
                    if (point.getClaimer() != null) {
                        g.setFill(playerColors.get(point.getClaimer()));
                        g.fillRect(pointXLoc + ((1 - percentFillSquareForTrailPoint) / 2) * pixlesPerSplixPoint,
                                ...
                                percentFillSquareForTrailPoint * pixlesPerSplixPoint);
                    } else if (point.getOwner() != null) {
                        g.setFill(playerColors.get(point.getOwner()));
